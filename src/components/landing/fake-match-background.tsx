...
function drawBoard(ctx: CanvasRenderingContext2D, visual: VisualState) {
  ...
  for (let row = 0; row < BOARD_CELLS; row += 1) {
    for (let col = 0; col < BOARD_CELLS; col += 1) {
      ...
      const isLastMove = visual.lastMoveKey === moveKey;
  ...
  return (
    <div aria-hidden className="pointer-events-none absolute inset-0 overflow-hidden">
      ...
      <canvas
        ...
        width={BOARD_SIZE}
        height={BOARD_SIZE}
        className="absolute left-1/2 top-1/2 h-[132vmax] w-[132vmax] -translate-x-1/2 -translate-y-1/2 opacity-80 blur-[2px] [mask-image:radial-gradient(circle,black_56%,transparent_94%)] [animation:board-drift_22s_ease-in-out_infinite_alternate]"
      />
    </div>
  );
}
